...
class BookingService
{
    ...
    protected function calculateBookingData($subscribePackage, $validatedData)
    {
        ...
        $startedAt = now();
        $endedAt = $startedAt->copy()->addDays($duration);
    ...
    {
        $booking = $this->bookingRepository->getBookingFromSession();
    ...
    {
        Log::info("payment store booking services");
        $bookingData = $this->bookingRepository->getBookingFromSession();
        if (!$bookingData) {
        ...
        DB::transaction(function () use ($validated, &$bookingTransactionId, $bookingData) {
            ...
            $validated['price'] = $bookingData['sub_total'];
            $validated['total_tax_amount'] = $bookingData['total_ppn'];
